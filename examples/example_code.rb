...
class ExampleRunner
  ...
  def self.execute(message, use_proxy = false)
    ...
    message.from_email_address = EmailAddress.new("david.schrenker@example.com")

    if message.instance_of? BasicMessage
      # message.add_to_email_address("yourname@example.com")
      message.add_to_email_address("david.schrenker@socketlabs.com")
      message.add_to_email_address("dschrenker@gmail.com")
    end
